...
public:
	...
	{
		//DONE: 메모리풀에서 할당해서 리턴
		return reinterpret_cast<T*>(GMemoryPool->Allocate(sizeof(T)*n));
	}
...
struct xdeque
{
	//DONE: STL 할당자를 사용하는 deque를 type으로 선언
	//typedef ... type;
...
struct xmap
{
	//DONE: STL 할당자 사용하는 map을  type으로 선언
	//typedef ... type;
	typedef std::map < K, T, C, STLAllocator< std::pair <const K, T> > > type;
};

template <class T, class C = std::less<T> >
struct xset
{
	//DONE: STL 할당자 사용하는 set을  type으로 선언
	//typedef ... type;
...
{
	//DONE: STL 할당자 사용하는 priority_queue을  type으로 선언
	//typedef ... type;
